...
export function FeatureImportanceChart({ explanation }: FeatureImportanceChartProps) {
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>Key parameters influencing the recommendation</CardDescription>
      </CardHeader>
      <CardContent>
            ...
            <BarChart data={data} layout="vertical" margin={{ left: 10, right: 20 }}>
              ...
              <Bar dataKey="value" radius={[0, 6, 6, 0]} barSize={20}>
                {data.map((entry, index) => (
                  <Cell 
                    key={`cell-${index}`} 
                    fill={index === 0 ? "hsl(var(--primary))" : "hsl(var(--accent)/0.6)"} 
                  />
                ))}
              </Bar>
            </BarChart>
